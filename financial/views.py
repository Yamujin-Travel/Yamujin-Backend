...
from constants import DEPOSIT_API_URL, SAVING_API_URL
...
def contract_deposit(request, deposit_code):
    ...
    if request.method == 'GET':
        ...
        if request.user not in deposit.contract_user.all():
            ...
            if serializer.is_valid(raise_exception=True):
                serializer.save()
                return Response({ "detail": "상품이 추가되었습니다." }, status=status.HTTP_201_CREATED)
        else:
...
def get_bank_deposit(request, kor_co_nm):
    if Deposit.objects.filter(kor_co_nm=kor_co_nm).exists():
        ...
        return Response({ "detail": "해당은행의 상품이 없습니다.." }, status=status.HTTP_404_NOT_FOUND)
